...
def projection(data_path, output_video_path, max_frames=500, fps=25, camera_height=260, camera_width=346,):
    ...
    for prop_name in props_names:
        with open(join(data_path, info_json['prop_marker_files'][prop_name]), 'r') as marker_file:
            markers = json.load(marker_file)

        props_markers[prop_name] = markers
        mesh = stl.mesh.Mesh.from_file(join(data_path, info_json['prop_mesh_files'][prop_name])).vectors.transpose(0, 2, 1)
